...
public class FirstPersonCameraController : MonoBehaviour
{
    ...
    public float runZOffset = 0.05f;        // Forward offset when running
    public float runYOffset = -0.05f;       // Downward offset when running

    [Header("Camera Jump Offset")]
    public float jumpZOffset = 0f;          // Forward offset when jumping
    public float jumpYOffset = 0f;          // Downward offset when jumping

    [Header("Camera Crouch Offset")]
    public float crouchZOffset = 0f;        // Forward offset when crouching
    public float crouchYOffset = -0.55f;    // Downward offset when crouching

    [Header("Camera Attack Offset")]
    public float attackZOffset = 0f;        // Forward offset when attacking
